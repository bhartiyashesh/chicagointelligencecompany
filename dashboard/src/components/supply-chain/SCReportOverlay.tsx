...
import type { SupplyChainReportData } from "@/lib/types";
import { ReportKVSection, ReportTableSection } from "../ReportSection";

interface Props {
  report: SupplyChainReportData;
  runId: string;
  onClose: () => void;
}
...
export default function SCReportOverlay({ report, runId, onClose }: Props) {
  ...
  const handlePrintPDF = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${companyName} — /cic Supply Chain Intelligence</title>
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cover { page-break-after: always; height: 100vh; display: flex; flex-direction: column; justify-content: space-between; padding: 3in 1.2in 1.5in; background: #0a0a0a; }
  .cover-top { }
  ...
  .cover-logo .nm { font-size: 11px; font-weight: 900; color: #fff; letter-spacing: -0.02em; }
  .cover h1 { font-size: 42px; font-weight: 800; color: #fff; letter-spacing: -0.04em; line-height: 1.05; margin-bottom: 12px; }
  .cover .subtitle { font-size: 15px; color: #F97316; font-weight: 500; margin-bottom: 8px; }
  .cover .price { font-size: 12px; color: #666; font-weight: 400; margin-bottom: 40px; }
  .cover .meta { font-size: 10px; color: #666; line-height: 1.8; }
  .cover .meta span { display: block; }
  .cover-bottom { border-top: 1px solid #333; padding-top: 16px; display: flex; justify-content: space-between; align-items: flex-end; }
  .cover-bottom .left { font-size: 9px; color: #666; text-transform: uppercase; letter-spacing: 0.15em; }
  .cover-bottom .right { font-size: 9px; color: #555; }
  ...
  .kv-val { padding: 10px 0; color: #222; font-size: 10.5pt; line-height: 1.6; border-bottom: 1px solid #f0efe8; }

  .back-page { page-break-before: always; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; background: #0a0a0a; }
  .back-page .sl { font-size: 96px; font-weight: 200; color: #F97316; line-height: 1; }
  .back-page .nm { font-size: 18px; font-weight: 900; color: #fff; margin-top: -12px; letter-spacing: -0.02em; }
  .back-page .full { font-size: 11px; color: #666; margin-top: 12px; letter-spacing: 0.12em; text-transform: uppercase; }
  .back-page .disc { font-size: 8px; color: #555; margin-top: 48px; max-width: 400px; line-height: 1.6; }
    ...
    w.document.write(`
      <div class="cover">
        <div class="cover-top">
          ...
          <h1>${companyName}</h1>
          <div class="subtitle">Supply Chain Intelligence Report</div>
          <div class="price">$75 per analysis</div>
          <div class="meta">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-5xl max-h-[92vh] bg-[#f5f5f0] rounded-xl border border-neutral-200 shadow-[0_8px_60px_rgba(0,0,0,0.25)] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-8 py-5 border-b border-neutral-200 bg-white/60 shrink-0">
          <div>
            ...
            {report.executive_summary?.company_name && (
              <p className="text-xl font-light text-[#F97316] ml-[52px]">
                {report.executive_summary.company_name}
              </p>
            )}
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <a
              ...
              className="px-4 py-1.5 rounded-lg border border-[#F97316]/30 text-[#F97316] text-[10px] font-semibold tracking-[0.15em] uppercase hover:bg-[#F97316]/10 transition-all duration-200 flex items-center gap-1.5"
            >
              <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M6 1v8M3 6l3 3 3-3" />
                <path d="M1 10h10" />
            ...
            {["xlsx", "csv", "json"].map((fmt) => (
              <a
                ...
                className="px-3.5 py-1.5 rounded-lg bg-white border border-neutral-200 text-[10px] font-semibold text-neutral-500 tracking-[0.15em] uppercase hover:text-[#F97316] hover:border-[#F97316]/30 transition-all duration-200 hidden sm:inline-block"
              >
                {fmt}
              </a>
            ))}
            <button
              onClick={onClose}
              className="ml-1 sm:ml-3 w-8 h-8 rounded-lg bg-white border border-neutral-200 flex items-center justify-center text-neutral-400 hover:text-neutral-800 hover:border-neutral-400 transition-all cursor-pointer"
            >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 bg-[#f5f5f0]">
          <div ref={printRef} className="print-content">
            ...
            {report.supplier_map && report.supplier_map.length > 0 && (
              <ReportTableSection title="Supplier Map" data={report.supplier_map} />
            )}
            {report.geographic_risk && (
              <ReportKVSection title="Geographic Risk" data={report.geographic_risk} />
            )}
            {report.dependency_analysis && report.dependency_analysis.length > 0 && (
              <ReportTableSection title="Dependency Analysis" data={report.dependency_analysis} />
            )}
            {report.logistics_infrastructure && (
              <ReportKVSection title="Logistics Infrastructure" data={report.logistics_infrastructure} />
            )}
            {report.trade_regulatory_risk && (
              <ReportKVSection title="Trade & Regulatory Risk" data={report.trade_regulatory_risk} />
            )}
            {report.competitive_displacement && report.competitive_displacement.length > 0 && (
              <ReportTableSection title="Competitive Displacement" data={report.competitive_displacement} />
            )}
            {report.scenarios && report.scenarios.length > 0 && (
              <ReportTableSection title="Scenarios" data={report.scenarios} />
            )}
            {report.recommendations && (
              <ReportKVSection title="Recommendations" data={report.recommendations} />
            )}
